...
myApp.options.init = function(){
    ...
    myApp.loadTemplate('options',{notesColor:localStorage.notesColor || 'F8F8FF',couchDbUrl:localStorage.couchDbUrl},'#mainView');
    myApp.options.listenForEvents();
    
    myApp.options.listenForEvents = function () {
        var softClose = false;
        myApp.optionsWindow.on('close',function(){
            if( !softClose && confirm ("Save the data before closing?")) myApp.saveData();
            this.hide();
            this.close(true);
        });
        $('#save').click(function(){
